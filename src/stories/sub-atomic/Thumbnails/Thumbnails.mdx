...
It uses a 2-layer Shadow DOM architecture combined with an automated canvas flood-fill mask generator that dynamically clips screen media to device bezels with subpixel precision.
...
1. **Screen Media Layer (`.screen-media`, z-index: 1):** Renders `screen-image` or `screen-video` inside the same bounded/masked display region.
...
The `<ds-thumbnail>` component accepts the following attributes for frame selection, media binding, and state control:
...
<table style={{ width: '100%', borderCollapse: 'collapse' }}>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><code>''</code></td>
      <td>Primary image URL rendered inside the device display bounds. If both media attributes are set, this source wins.</td>
    </tr>
    <tr>
      <td><code>screen-video</code></td>
      <td><code>URL String</code></td>
      <td><code>''</code></td>
      <td>Optional video URL for the same display bounds. Used only when <code>screen-image</code> is not provided.</td>
    </tr>
    <tr>
...
Media precedence: when both <code>screen-image</code> and <code>screen-video</code> are passed, <code>screen-image</code> is rendered.
...
<ds-thumbnail
  ...
  model="Apple iPhone 13"
  color="Midnight"
  screen-video="https://interactive-examples.mdn.mozilla.net/media/cc0-videos/flower.mp4"
  max-height="500px">
</ds-thumbnail>
