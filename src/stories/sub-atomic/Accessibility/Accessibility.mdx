...
The canonical color and contrast values live in [accessibility.css](./accessibility.css). The package entry re-exports a derived token map for consumers that need runtime access, but token edits should still be made in the CSS file.

<br />

## Color & Contrast System

To eliminate theme complexity and prevent color transition drift, the accessibility stylesheet defines theme overrides for four core semantic roles (`--color-bg`, `--color-surface`, `--color-text-primary`, and `--color-surface-border`) rather than individual component primitives. The exported JS token map is derived from those CSS rules, so the CSS remains the source of truth.
...
The same host-attribute pattern applies to forced-colors support:
